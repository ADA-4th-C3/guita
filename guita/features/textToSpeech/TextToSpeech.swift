...
final class TextToSpeech: NSObject, ObservableObject, @unchecked Sendable {
  ...
  // TTS 속도 설정 (간단화)
  var currentSpeechRate: Float = 0.5  // 초기값 0.5
  
  // 속도 조절 범위
  private let minSpeechRate: Float = 0.3  // 하한
  private let maxSpeechRate: Float = 0.7  // 상한
  private let speedStep: Float = 0.1      // 조절 단위
  ...
  func speak(
    ...
    rate: Float = AVSpeechUtteranceDefaultSpeechRate,  // 시스템 기본 속도
    ...
    if language.contains("ko") {
      // 가장 기본적인 한국어 음성만 선택
      selectedVoice = AVSpeechSynthesisVoice(language: "ko-KR")
      Logger.d("TTS: 기본 한국어 음성 사용")
    } else {
      // 영어나 기타 언어도 기본 음성만
...
extension TextToSpeech: AVSpeechSynthesizerDelegate {
  ...
  /// TTS 기본 속도 설정 (0.1 ~ 1.0)
  func setDefaultSpeechRate(_ rate: Float) {
    currentSpeechRate = max(0.1, min(1.0, rate))
    Logger.d("TTS 기본 속도 변경: \(currentSpeechRate)")
  }
  
  /// TTS 속도 올리기 (0.1씩 증가, 최대 0.7)
  func increaseTTSSpeed() {
    let newRate = min(maxSpeechRate, currentSpeechRate + speedStep)
    if newRate != currentSpeechRate {
      currentSpeechRate = newRate
      Logger.d("TTS 속도 증가: \(currentSpeechRate)")
    } else {
      Logger.d("TTS 속도 최대값 도달: \(currentSpeechRate)")
    }
  }
  
  /// TTS 속도 내리기 (0.1씩 감소, 최소 0.3)
  func decreaseTTSSpeed() {
    let newRate = max(minSpeechRate, currentSpeechRate - speedStep)
    if newRate != currentSpeechRate {
      currentSpeechRate = newRate
      Logger.d("TTS 속도 감소: \(currentSpeechRate)")
    } else {
      Logger.d("TTS 속도 최소값 도달: \(currentSpeechRate)")
    }
  }
